...
class Imager:
    ...
    def pyralysis_start_client(self, config):
        try:
            ...
            from ..helpers.pyralysis.setup_client import setup_optimal_client
            from ..helpers.pyralysis.mask_modified import MaskModified

            import dask.array as da
            import xarray as xr
        except ImportError as e:
            print(f"Error importing Pyralysis modules: {e}")
        ...
        self.GaussianSinc = GaussianSinc
        self.Sinc = Sinc
        self.Degridding = Degridding
        ...
        self.Brent = Brent
        self.ZarrArray = ZarrArray
        self.LessThan = LessThan
        self.PolakRibiere = PolakRibiere
        self.HagerZhang = HagerZhang
        self.LBFGS = LBFGS
        self.FletcherReeves = FletcherReeves
        self.HestenesStiefel = HestenesStiefel
        self.DaiYuan = DaiYuan
        self.SDMM = SDMM
        self.DaskMS = DaskMS
        self.HermitianSymmetry = HermitianSymmetry
        self.MaskModified = MaskModified

        self.da = da
        self.xr = xr

    def pyralisis_stop_client(self):
        if self.client is not None:
            self.client.shutdown()
            self.client = None
            self.n_workers = 0

    def pyralysis_setup_dataset(self, config):
        ...
        ds = self.DaskMS(input_name=self.vis)
        ...
        if hermitian_symmetry:
            self.h_symmetry = self.HermitianSymmetry(input_data=self.dataset)
        ...
        image_start = self.da.zeros((self.imsize, self.imsize), chunks=chunks, dtype=np.float32)
        self.image = self.Image(
            data=self.xr.DataArray(
                data=image_start,
                dims=["x", "y"],
    ...
    def pyralysis_reconstruct(self, config):
        ...
        kernel_classes = {
            ...
            "gaussian_sinc": self.GaussianSinc,
            "sinc": self.Sinc,
        ...
        estimator_classes = {
            "nearest_neighbour": self.NearestNeighbor,
            "bilinear_interpolation": self.BilinearInterpolation,
            "degridding": self.Degridding
        }
        ...
        chi_squared = self.Chi2(model_visibility=estimator, normalize=True)
        l1_norm = self.L1Norm(penalization_factor=1e-3)
        tsv = self.TSV(penalization_factor=1e-20)
        fi_list = [chi_squared, l1_norm, tsv]

        of = self.ObjectiveFunction(fi_list=fi_list, image=self.image, persist_gradient=True)
        ...
        line_search_classes = {
            "fixed": self.Fixed,
            "backtracking_armijo": self.BacktrackingArmijo,
            "brent": self.Brent
        }
        ...
        output_file = f"{self.output_path}/{optimizer_algorithm}_{niter_optimizer}.jpg"
